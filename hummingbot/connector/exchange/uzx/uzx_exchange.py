...
class UzxExchange(ExchangePyBase):
    ...
    async def _place_order(self,
        ...
        if order_type is OrderType.MARKET:
            # Explicit, configurable semantics for MARKET orders
            is_buy = (trade_type is TradeType.BUY)

            if self._market_trade_ccy == "quote":
                # Send quote notional with trade_ccy=1
                ref_px = await self._reference_price(trading_pair, is_buy=is_buy)
                quote_notional = (amount * ref_px)  # amount passed in is base qty; convert to quote notional
                api_params["trade_ccy"] = 1
                api_params["amount"] = f"{quote_notional:f}"
            else:
                # Send base quantity with trade_ccy=0 (use if UZX accepts base-mode MARKET)
        ...
        try:
            ...
            if simulate_ioc:
                self._log_event("DEBUG", "order.ioc.cancel.scheduled", coid=order_id, xo=o_id, delay_ms=self._ioc_cancel_delay_ms)
                try:
                    await asyncio.sleep(self._ioc_cancel_delay_ms / 1000)
